...
Continue adding GCPs. For this historical map, I recommend primarily using street intersections as GCPs. Begin around the periphery and then add some along major crossroads towards the center of Vancouver. I also recommend anchoring the shoreline by adding GCPs on either side of the three bridges. 
...
In the Georeferencer Window table the column dX, dY and Residual refer to error. You may notice little red lines on your Source layer around the GCP point. These are the error vectors visualizing dX and dY. The longer the line, the greater the error. For this exercise, the error should be small and you can ignore the lines. If you get any really long ones, double check that your GCPs match. The error is likely the result of misplacing the GCP on your target layer. <!--note about shoreline and error, gcps around bridge etc -->
<!-- 
In the status bar of the Georeferencer Window you should see Mean Error. 

root mean square error (RMSE) vs mean error (whats given)
